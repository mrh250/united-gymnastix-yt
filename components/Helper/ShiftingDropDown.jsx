...
export const ShiftingDropDown = () => {
  return (
    <div className="flex h-96 w-full justify-start bg-yellow-400 p-8 text-black md:justify-center">
      <Tabs />
    </div>
  );
};
...
const Tabs = () => {
  ...
  const handleSetSelected = (val) => {
    console.log("Setting selected:", val); //Debug
...
const Tab = ({ children, tab, handleSetSelected, selected }) => {
  return (
    ...
    <button
      id={`shift-tab-${tab}`}
      onMouseEnter={() => !isTouchDevice && handleSetSelected(tab)}
      onClick={() => isTouchDevice && handleSetSelected(tab)}
    ...
    <div>
      <div className="flex gap-4">
        <div>
          ...
          <a href="#" className="mb-1 block text-sm text-black">
            Bookkeeping
          </a>
...
const Pricing = () => {
  return (
    <div className="grid grid-cols-3 gap-4 divide-x divide-neutral-700">
      <a
        href="#"
        className="flex w-full flex-col items-center justify-center py-2 text-black transition-colors hover:text-neutral-50"
      >
